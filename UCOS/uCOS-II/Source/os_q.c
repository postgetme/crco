...
void  *OSQPend (OS_EVENT *pevent, INT16U timeout, INT8U *err)
{
    ...
    if (OSTCBCur->OSTCBStatPend != OS_STAT_PEND_OK) { /* Was task readied because timed-out or aborted?*/
        ...
        switch (pend_stat) {
            ...
            default:
                 *err = OS_ERR_TIMEOUT;          /*     Indicate a timeout occured                     */
                 break;
...
INT8U  OSQPendAbort (OS_EVENT *pevent, INT8U opt, INT8U *err)
{
    INT8U      nbr_tasks;
#if OS_CRITICAL_METHOD == 3                                /* Allocate storage for CPU status register */
    OS_CPU_SR  cpu_sr = 0;
...
INT8U  OSQPost (OS_EVENT *pevent, void *msg)
{
    ...
    if (pevent->OSEventGrp != 0) {                     /* See if any task pending on queue             */
        ...
        (void)OS_EventTaskRdy(pevent, msg, OS_STAT_Q, OS_STAT_PEND_OK);
        OS_EXIT_CRITICAL();
        OS_Sched();                                    /* Find highest priority task ready to run      */
        return (OS_ERR_NONE);
    }
