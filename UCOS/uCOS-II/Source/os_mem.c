...
OS_MEM  *OSMemCreate (void *addr, INT32U nblks, INT32U blksize, INT8U *err)
{
    ...
    if ((blksize % sizeof(void *)) != 0) {            /* Must contain space for an integral number ... */
        *err = OS_ERR_MEM_INVALID_SIZE;               /* ... of pointer sized items                    */
        return ((OS_MEM *)0);
    }
#endif
    OS_ENTER_CRITICAL();
...
void  *OSMemGet (OS_MEM *pmem, INT8U *err)
{
    ...
    if (pmem->OSMemNFree > 0) {                       /* See if there are any free memory blocks       */
        ...
        OS_EXIT_CRITICAL();
        *err = OS_ERR_NONE;                           /*      No error                                 */
        return (pblk);                                /*      Return memory block to caller            */
    }
    OS_EXIT_CRITICAL();
    *err = OS_ERR_MEM_NO_FREE_BLKS;                   /* No,  Notify caller of empty memory partition  */
