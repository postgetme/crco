...
unsigned int  gProgramRomAddr;

#define APP_PROGRAM_ROM_ADDR    gProgramRomAddr
...
//*****************************************************************************
#if defined(ewarm)
static __no_init void (*g_pfnRAMVectors[NUM_INTERRUPTS])(void) @ 0x20000800L;   // "VTABLE";
#elif defined(sourcerygxx)
static __attribute__((section(".cs3.region-head.ram")))
void (*g_pfnRAMVectors[NUM_INTERRUPTS])(void);
#else
static __attribute__((section("vtable")))
void (*g_pfnRAMVectors[NUM_INTERRUPTS])(void) __attribute__((at(0x20000800L)));
#endif
...
IntRegister(unsigned long ulInterrupt, void (*pfnHandler)(void))
{
    ...
    if(HWREG(NVIC_VTABLE) != (unsigned long)g_pfnRAMVectors)
    {
        ...
        {
            g_pfnRAMVectors[ulIdx] = (void (*)(void))HWREG(ulIdx * 4 + APP_PROGRAM_ROM_ADDR);
        }
...
//*****************************************************************************
//
//! 将BOOT的中断向量表注册到应用程序中来
//!
//! 注：在调试应用程序的时候必须先下载BOOTLOAD，如果不下载BOOTLOAD，应用程序不能
//!     进行调试。
//! \param 无
//!
//! \return 无
//！tangjun 20090604增加该函数  
//*****************************************************************************
void 
IntSysBootRegister(unsigned int RomAddr)
{
    unsigned long ulIdx;

	gProgramRomAddr = RomAddr;
